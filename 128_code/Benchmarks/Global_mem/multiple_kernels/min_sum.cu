...
__global__ void GPU_min_sum_equations_and_decision(unsigned short * d_sindrome, unsigned char * d_LPi, unsigned char * d_wrong_equations, unsigned char * teta, unsigned char * bar){
    ...
    /* if(threadIdx.x==0){
        for(int i=0;i<128;i++)
        printf("d_wrong_equations[%d]=%d, k=%d\n", i, d_wrong_equations[i], 5*64+(i-64)*3);  
    }
    __syncthreads(); */
...
int  main(){
    ...
    for(int num_iter=0; num_iter<MAX_ITER;num_iter++){
        ...
        GPU_min_sum_equations_and_decision<<<numBlocks, threadsPerBlock_equations_and_decision>>>(d_sindrome, d_LPi, d_wrong_equations, teta, bar);
        if(err!=cudaSuccess){
            fprintf(stderr, "Failed to launch main kernel (error code %d)!\n", cudaGetLastError());
            exit(EXIT_FAILURE);
        }
