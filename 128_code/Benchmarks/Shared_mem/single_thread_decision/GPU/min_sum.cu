...
__global__ void GPU_min_sum_bar(unsigned char * teta, unsigned char * bar);
__global__ void GPU_min_sum_sindrome(unsigned short * d_sindrome, unsigned char * d_LPi);
__global__ void GPU_min_sum_equations_and_decision(unsigned short * d_sindrome, unsigned char * d_LPi, unsigned char * d_wrong_equations, unsigned char * teta, unsigned char * bar);
...
int  main(){
    ...
    dim3 threadsPerBlock_bar(1,1,1);
    ...
    for(int num_iter=0; num_iter<MAX_ITER;num_iter++){
        ...
        GPU_min_sum_bar<<<numBlocks, threadsPerBlock_bar>>>( teta, bar);
        if(err!=cudaSuccess){
            fprintf(stderr, "Failed to launch bar kernel (error code %d)!\n", cudaGetLastError());
            exit(EXIT_FAILURE);
        }
