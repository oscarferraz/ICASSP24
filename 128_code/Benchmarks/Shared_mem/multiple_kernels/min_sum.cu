...
__global__ void GPU_min_sum_sindrome(unsigned short * d_sindrome, unsigned char * d_LPi, unsigned char * teta, unsigned char * bar);
__global__ void GPU_min_sum_equations_and_decision(unsigned short * d_sindrome, unsigned char * teta, unsigned char * bar);
...
__global__ void GPU_min_sum_sindrome(unsigned short * d_sindrome, unsigned char * d_LPi, unsigned char * teta, unsigned char * bar){
    ...
    __shared__ unsigned char s_bar[1];

    s_LPi[threadIdx.x*2]=d_LPi[threadIdx.x*2];
    s_LPi[threadIdx.x*2+1]=d_LPi[threadIdx.x*2+1];
    s_bar[0]=bar[0];
...
//**************************************************************************************************
//Kernel
__global__ void GPU_min_sum_equations_and_decision(unsigned short * d_sindrome, unsigned char * d_LPi, unsigned char * teta, unsigned char * bar){

    __shared__ unsigned short s_sindrome[M];
    __shared__ unsigned char s_wrong_equations[N];

    if(threadIdx.x<64)
        s_sindrome[threadIdx.x]=d_sindrome[threadIdx.x];
    __syncthreads();
    ...
    /* if(threadIdx.x==0){
        for(int i=0;i<128;i++)
        printf("d_wrong_equations[%d]=%d, k=%d\n", i, s_wrong_equations[i], 5*64+(i-64)*3);  
    }
    __syncthreads(); */
...
int  main(){
    ...
    for(int num_iter=0; num_iter<MAX_ITER;num_iter++){
        GPU_min_sum_sindrome<<<numBlocks, threadsPerBlock_sindrome>>>(d_sindrome, d_LPi, teta, bar);
        if(err!=cudaSuccess){
            fprintf(stderr, "Failed to launch main kernel (error code %d)!\n", cudaGetLastError());
            exit(EXIT_FAILURE);
        }

        GPU_min_sum_equations_and_decision<<<numBlocks, threadsPerBlock_equations_and_decision>>>(d_sindrome, d_LPi, teta, bar);
        if(err!=cudaSuccess){
            fprintf(stderr, "Failed to launch main kernel (error code %d)!\n", cudaGetLastError());
            exit(EXIT_FAILURE);
        }
