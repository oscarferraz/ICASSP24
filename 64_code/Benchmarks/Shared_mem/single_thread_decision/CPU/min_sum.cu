...
__global__ void GPU_min_sum_equations_and_decision(unsigned short * d_sindrome, unsigned char * d_LPi, unsigned char * d_teta, unsigned char * d_bar);
...
int  main(){
    ...
    for(int num_iter=0; num_iter<MAX_ITER;num_iter++){
        ...
        GPU_min_sum_equations_and_decision<<<numBlocks, threadsPerBlock_equations_and_decision>>>(d_sindrome, d_LPi, d_teta, d_bar);
        if(err!=cudaSuccess){
