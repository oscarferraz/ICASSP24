  ...
  // ------------------------------------------------------------------- //
	...
	for (snr=init_snr; snr<=final_snr; snr+=snr_increment){
		...
		for(i=0; i<num_sim; i++){
			...
			DPU_CHECK(dpu_broadcast_to(set, "L", 0, LPi, 256*sizeof(char),DPU_XFER_DEFAULT), errno);
			...
			char m_decoded_word[256*nr_dpus];
			...
			DPU_FOREACH(set, dpu, each_dpu) {
				...
				DPU_ASSERT(dpu_prepare_xfer(dpu, &m_decoded_word[256*each_dpu]));
			}
			DPU_ASSERT(dpu_push_xfer(set, DPU_XFER_FROM_DPU, "L", 0, 256*sizeof(char), DPU_XFER_DEFAULT));
			...
			for (i=0; i<256 ;i++){
				...
				for (int j=0; j<nr_dpus ;j++){
					sum=sum+m_decoded_word[(j*256)+i];
				}

				if((sum != 0) && (sum != nr_dpus)){
					printf("error at %d, value1=%d, value2=%d, sum=%d\n",  i, m_decoded_word[i], m_decoded_word[256+i], sum );
				}
