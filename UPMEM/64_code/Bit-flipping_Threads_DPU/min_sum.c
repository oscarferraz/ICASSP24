...
//char   *decoded_word;
char   decoded_word[128];				  // --> word decoded by the LDPC decoder
float  snr_rms;
					   ...
					   int seed)
{
	...
	if( (LPi =(char *) malloc(128*sizeof(char))) == NULL ){
  ...
  // ------------------------------------------------------------------- //
	...
	DPU_CHECK(dpu_alloc(DPU_ALLOCATE_ALL, NULL, &set), errno);
	...
	for (snr=init_snr; snr<=final_snr; snr+=snr_increment){
		...
		for(i=0; i<num_sim; i++){
			...
			DPU_CHECK(dpu_broadcast_to(set, "L", 0, LPi, 128*sizeof(char),DPU_XFER_DEFAULT), errno);
			...
			char m_decoded_word[128*nr_dpus];
			uint32_t nb_cycles[nr_dpus];
			...
			/* DPU_FOREACH(set, dpu) {
				...
				DPU_CHECK(dpu_copy_from(dpu, "decoded_word", 0, &m_decoded_word, 128*sizeof(char)), errno);
			...
			DPU_FOREACH(set, dpu, each_dpu) {
				...
				DPU_ASSERT(dpu_prepare_xfer(dpu, &m_decoded_word[128*each_dpu]));
			}
			DPU_ASSERT(dpu_push_xfer(set, DPU_XFER_FROM_DPU, "L", 0, 128*sizeof(char), DPU_XFER_DEFAULT));
			...
			for (i=0; i<128 ;i++){
				...
				for (int j=0; j<nr_dpus ;j++){
					sum=sum+m_decoded_word[(j*128)+i];
				}

				if((sum != 0) && (sum != nr_dpus)){
					printf("error at %d, value1=%d, value2=%d, sum=%d\n",  i, m_decoded_word[i], m_decoded_word[128+i], sum );
				}
				
			} 

			printf("%lf %d\n",(end.tv_sec-start.tv_sec)*1e3+(end.tv_nsec-start.tv_nsec)*1e-6, nb_cycles[0]);
